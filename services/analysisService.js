...
export const performFullAnalysis = async (listingId) => {
  ...
  try {
    ...
    if (report.score === 0) {
      ...
      report.details.image_check =
        aiResult.verdict === "Rejected" ? "failed" : "passed";
    }
    ...
    if (
      (userRole === "agencyagent" || userRole === "independentagent") &&
      data.country &&
      ...
      if (listingC !== profileC) {
        ...
        report.score -= 15;
        report.flags.push(
          `Agent Location (${data.profile_country}) does not match Property Country.`,
        );
      }
    }
